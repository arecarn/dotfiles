...
- **Claude `@` imports resolve inside this repo, and fail silently:** `~/.claude/CLAUDE.md` is a symlink here, so `@references/foo.md` resolves next to the real file in this repo, not in `~/.claude/references/`. A private reference from `dotfiles_local` needs the gitignored bridge link that `dotfiles_local`'s stow creates, and imports are not expanded recursively — see [docs/gotchas/claude-user-level-imports-resolve-next-to-the-symlink-target.md](docs/gotchas/claude-user-level-imports-resolve-next-to-the-symlink-target.md) and [docs/gotchas/claude-at-imports-do-not-recurse.md](docs/gotchas/claude-at-imports-do-not-recurse.md).
